...
//let t1 = 0;
     ...
     var c = document.getElementById("canvas");
        ...
        ctx.scale(0.5,0.5);
         ...
         var speedGradient = ctx.createLinearGradient(0, 250, 0, 0);
        speedGradient.addColorStop(0, '#00b8fe');
        speedGradient.addColorStop(1, '#41dcf4');

        var rpmGradient = ctx.createLinearGradient(0, 250, 0, 0);
        ...
        function drawSpeedo(speed, topSpeed, gear, rpm ) {
            if (speed == undefined) {
                return false;
            } else {
           ...
           ctx.fillStyle = "#FFF";
            for (var i = 10; i <= Math.ceil(topSpeed / 20) * 20; i += 10) {
                ...
                if(i<=100) { 
                    drawMiniNeedle(calculateSpeedAngle(Math.floor(i) / 67, 0, 22.9) * Math.PI, i % 20 == 0 ? 3 : 1, i % 20 ==
                    0 ?
                    Math.floor(i / 14) : '');
                }
            }
...
function onDOMContentLoaded() {
    ...
    //renderCanvas(); drawSpeedo(120,160,5,5);
    drawSpeedo(0,140,1,0);
}
